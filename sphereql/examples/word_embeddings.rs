...
fn word_table() -> Vec<(&'static str, &'static str, Embedding)> {
    ...
    vec![
        // --- Royalty ---
        ("king",      "royalty",  embed(&[(HUMAN,0.9),(ROYALTY,1.0),(MASCULINE,0.9),(POWER,0.9),(WEALTH,0.9),(MATURE,0.7),(INTELLECT,0.6)], 1)),
        ("queen",     "royalty",  embed(&[(HUMAN,0.9),(ROYALTY,1.0),(MASCULINE,-0.8),(POWER,0.8),(WEALTH,0.9),(BEAUTY,0.6),(INTELLECT,0.6)], 2)),
        ("prince",    "royalty",  embed(&[(HUMAN,0.9),(ROYALTY,0.8),(MASCULINE,0.8),(POWER,0.5),(WEALTH,0.8),(MATURE,-0.3)], 3)),
        ("princess",  "royalty",  embed(&[(HUMAN,0.9),(ROYALTY,0.8),(MASCULINE,-0.7),(BEAUTY,0.7),(WEALTH,0.8),(MATURE,-0.3)], 4)),
        ("throne",    "royalty",  embed(&[(ROYALTY,0.9),(POWER,0.8),(WEALTH,0.7),(HEIGHT,0.3)], 5)),
        ("crown",     "royalty",  embed(&[(ROYALTY,0.9),(WEALTH,0.8),(BRIGHT,0.5),(BEAUTY,0.5),(POWER,0.6)], 6)),

        // --- Animals ---
        ("cat",       "animal",  embed(&[(LIVING,0.9),(ANIMAL,1.0),(DOMESTIC,0.8),(PREDATORY,0.4),(SOFT,0.7),(SPEED,0.5),(SIZE,-0.3)], 7)),
        ("dog",       "animal",  embed(&[(LIVING,0.9),(ANIMAL,1.0),(DOMESTIC,0.9),(SOCIAL,0.7),(LOUD,0.5),(SPEED,0.5)], 8)),
        ("lion",      "animal",  embed(&[(LIVING,0.9),(ANIMAL,1.0),(PREDATORY,0.9),(POWER,0.8),(SIZE,0.6),(LOUD,0.7),(SPEED,0.6)], 9)),
        ("eagle",     "animal",  embed(&[(LIVING,0.8),(ANIMAL,0.9),(AERIAL,0.9),(PREDATORY,0.7),(SPEED,0.7),(HEIGHT,0.8),(BRIGHT,0.3)], 10)),
        ("fish",      "animal",  embed(&[(LIVING,0.8),(ANIMAL,0.9),(AQUATIC,1.0),(WET,0.8),(COLD,0.3),(SPEED,0.4),(FOOD,0.5)], 11)),
        ("whale",     "animal",  embed(&[(LIVING,0.8),(ANIMAL,0.9),(AQUATIC,0.9),(SIZE,1.0),(WET,0.7),(INTELLECT,0.5),(SOCIAL,0.4)], 12)),

        // --- Technology ---
        ("computer",  "tech",    embed(&[(TECH,1.0),(INTELLECT,0.6),(KNOWLEDGE,0.5),(BRIGHT,0.3),(POWER,0.2)], 13)),
        ("robot",     "tech",    embed(&[(TECH,0.9),(INTELLECT,0.5),(MOTION,0.5),(POWER,0.3),(HUMAN,0.2)], 14)),
        ("software",  "tech",    embed(&[(TECH,0.9),(INTELLECT,0.5),(KNOWLEDGE,0.4),(CREATIVE,0.3)], 15)),
        ("algorithm", "tech",    embed(&[(TECH,0.8),(INTELLECT,0.7),(KNOWLEDGE,0.6),(POWER,0.2)], 16)),

        // --- Nature ---
        ("tree",      "nature",  embed(&[(LIVING,0.8),(NATURE,1.0),(HEIGHT,0.6),(SIZE,0.5),(MATURE,0.5)], 17)),
        ("flower",    "nature",  embed(&[(LIVING,0.8),(NATURE,0.9),(BEAUTY,0.8),(BRIGHT,0.6),(SWEET,0.5),(SOFT,0.5)], 18)),
        ("mountain",  "nature",  embed(&[(NATURE,0.9),(HEIGHT,1.0),(SIZE,1.0),(COLD,0.5),(POWER,0.4)], 19)),
        ("river",     "nature",  embed(&[(NATURE,0.9),(WET,0.9),(MOTION,0.7),(COLD,0.2),(AQUATIC,0.5)], 20)),
        ("ocean",     "nature",  embed(&[(NATURE,0.8),(AQUATIC,0.9),(WET,1.0),(SIZE,1.0),(DARK,0.3),(POWER,0.5)], 21)),

        // --- Food ---
        ("bread",     "food",    embed(&[(FOOD,1.0),(WARMTH,0.5),(SOFT,0.5),(SWEET,0.2)], 22)),
        ("apple",     "food",    embed(&[(FOOD,0.9),(LIVING,0.4),(SWEET,0.6),(NATURE,0.3),(BRIGHT,0.3)], 23)),
        ("cake",      "food",    embed(&[(FOOD,0.9),(SWEET,0.9),(WARMTH,0.3),(BEAUTY,0.3),(CREATIVE,0.3)], 24)),
        ("pizza",     "food",    embed(&[(FOOD,0.9),(WARMTH,0.6),(SOCIAL,0.4),(CREATIVE,0.2)], 25)),
    ]
}
...
fn main() {
    ...
    println!("{:<12} {:<10} {:>8} {:>10} {:>10}", "word", "category", "r", "θ (deg)", "φ (deg)");
    println!("{}", "-".repeat(54));
    ...
    for query_word in ["king", "cat", "computer", "tree", "bread"] {
        ...
        for (i, res) in results.iter().enumerate() {
            ...
            let marker = if res.item.id == query_word { " ← (self)" } else { "" };
            println!(
                "  {}. {:<12} [{:<7}]  dist = {:.4} rad ({:>6.2}°){}",
                i + 1,
                res.item.id,
    ...
    println!("  ({} found, {} scanned)", similar.items.len(), similar.total_scanned);
    ...
    let queen_emb = words.iter().find(|(w,_,_)| *w == "queen").map(|(_,_,e)| e).unwrap();
