...
helpers used by example notebooks and downstream Python code. The
full text → `ConceptGraph` pipeline lives in the Rust crate
[`sphereql-lingua`](../sphereql-lingua/README.md) and is exposed to
Python via the `sphereql-python` bindings.
...
- `Concept`, `Relation`, `ConceptGraph`, `DomainAnchor` —
  dataclasses for hand-built or downstream-produced graphs.
...
Pre-1.0. Public surface kept small and stable; new functionality
ships in `sphereql-lingua` first and reaches Python through the
bindings.
