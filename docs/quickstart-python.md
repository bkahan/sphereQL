...
The Python bindings are built via PyO3/maturin and now cover the full
Rust surface: PCA, Kernel PCA, Laplacian eigenmap, UMAP-on-sphere
(via `config`), `auto_tune`, `MetaModel`, `FeedbackAggregator`, and
the category enrichment layer (concept paths, drill-down, domain
groups, stats).

Type stubs (`.pyi`) are auto-generated by `pyo3-stub-gen` — IDEs and
`mypy`/`pyright` pick them up automatically. Regenerate them with
`cargo run --bin gen-stubs --features vectordb` from `sphereql-python/`
...
The published wheel ships the in-memory vector store. The Qdrant and
Pinecone backends are not PyPI extras — build from source with the
matching Cargo feature:

```bash
pip install maturin
maturin develop --release --features qdrant    # or: --features pinecone
```
...
# Build a pipeline with a non-default projection kind. Valid kinds:
...
# UMAP-on-sphere, with its tunable min_dist (default 0.1):
pipeline = sphereql.Pipeline(
    categories,
    embeddings,
    config={"projection_kind": "UmapSphere", "umap": {"min_dist": 0.1}},
)
```
