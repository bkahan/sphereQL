...
impl PyLinguaPipeline {
    #[new]
    fn new() -> Self { Self { inner: sphereql_lingua::LinguaPipeline::new() } }
...
impl PyConceptGraph {
    #[getter]
    fn num_concepts(&self) -> usize { self.inner.concepts.len() }
    #[getter]
    fn num_relations(&self) -> usize { self.inner.relations.len() }

    fn concepts(&self) -> Vec<PyConcept> {
        self.inner.concepts.iter().map(PyConcept::from_concept).collect()
    }
    fn centroid(&self) -> Option<PySphericalPoint> {
        self.inner.centroid().map(PySphericalPoint::from_inner)
    }
    fn to_sphereql(&self) -> String { self.sphereql_cache.clone() }
    fn get_concept(&self, name: &str) -> Option<PyConcept> {
        self.inner.get_concept(name).map(PyConcept::from_concept)
    }
    fn __repr__(&self) -> String {
        format!("ConceptGraph(concepts={}, relations={})",
                self.inner.concepts.len(), self.inner.relations.len())
    }
    fn __len__(&self) -> usize { self.inner.concepts.len() }
}
...
pub struct PyConcept {
    #[pyo3(get)] pub normalized: String,
    #[pyo3(get)] pub text: String,
    #[pyo3(get)] pub frequency: u32,
    #[pyo3(get)] pub salience: f64,
...
#[pymethods]
impl PyConcept {
    #[getter] fn point(&self) -> Option<PySphericalPoint> { self.point.map(PySphericalPoint::from_inner) }
    #[getter] fn theta(&self) -> Option<f64> { self.point.map(|p| p.theta) }
    #[getter] fn phi(&self) -> Option<f64> { self.point.map(|p| p.phi) }
    #[getter] fn r(&self) -> Option<f64> { self.point.map(|p| p.r) }
    fn __repr__(&self) -> String {
        if let Some(p) = self.point {
            format!("LinguaConcept('{}', t={:.3}, p={:.3}, r={:.3})", self.normalized, p.theta, p.phi, p.r)
        } else {
...
impl PyConcept {
    fn from_concept(c: &sphereql_lingua::Concept) -> Self {
        Self { normalized: c.normalized.clone(), text: c.text.clone(),
               frequency: c.frequency, salience: c.salience_score, point: c.point }
    }
}
