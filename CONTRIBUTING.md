...
- Type stubs live at `sphereql-python/python/sphereql/__init__.pyi` and are
  auto-generated by the `gen-stubs` bin (`cargo run -p sphereql-python --bin
  gen-stubs --features vectordb`). Re-run after changing PyO3 signatures and
  commit the result.
...
This is the canonical list of local checks — the [README](README.md) and
[docs/testing.md](docs/testing.md) point here rather than keeping their
own copies. Each command maps to a CI job (see
[Branching and Pull Requests](#branching-and-pull-requests)).

### Running tests

```bash
# Full workspace with all features. sphereql-python is excluded — its
# extension-module feature breaks the `cargo test` link step (Python
# provides those symbols at load time); its tests run via pytest below.
cargo test --workspace --all-features --exclude sphereql-python

# Doc-tests
cargo test --doc --workspace --exclude sphereql-python

# A single crate, or a single test
cargo test -p sphereql-core
cargo test -p sphereql-embed -- kernel_pca::tests::kernel_pca_fit_default

# Lint + format (CI runs clippy with -Dwarnings)
cargo clippy --workspace --all-features --all-targets -- -D warnings
cargo fmt --all -- --check

# Drift checks: bindings parity (every public sphereql-embed/layout API
# needs a Python/WASM binding or an allowlist entry), and version-string
# consistency across every manifest, README, and doc.
