// SphereQL viewer runtime — extracted from template.html so the baked
// offline HTML and the (future) WASM studio share one implementation.
// Inlined verbatim into the emitted page at the /*__SPHEREQL_VIEWER__*/
// placeholder, immediately after the module-scope `const D` scene payload.
// Plain ES5/ES6 browser script (no modules/imports) — runs as-is inside the
// page <script> and reads the global `D`, `THREE`, and DOM nodes.
...
const posIndex=new Map();for(let i=0;i<N;i++)posIndex.set(pts[i].x+"|"+pts[i].y+"|"+pts[i].z,i);
const catDir={};
...
const overlayKinds=new Set(),bridgeLines=[],bridgesByPoint={},labelData=[];
overlays.forEach(o=>{
...
const labelsDiv=document.getElementById("labels"),labelEls=[];
let soloCat=null;
...
// Per-kind label toggles — one checkbox per label kind present in the scene.
const labelTogglesDiv=document.getElementById("label-toggles");
const labelKindsPresent=[...new Set(labelData.map(l=>l.kind))].sort();
...
const labelRefDist=DEF.scale*maxR*2.6;
