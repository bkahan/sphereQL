...
use sphereql_core::{
    SphericalPoint, angular_distance, cartesian_to_spherical, spherical_to_cartesian,
};
...
impl ConceptGraph {
    ...
    pub fn get_concept_idx(&self, normalized: &str) -> Option<usize> {
        self.concepts
            .iter()
            .position(|c| c.normalized == normalized)
    }

    pub fn centroid(&self) -> Option<SphericalPoint> {
        let resolved: Vec<&SphericalPoint> = self
            .concepts
            .iter()
    ...
    pub fn to_sphereql(&self, taxonomy: &DomainTaxonomy) -> String {
        ...
        let mut sorted: Vec<usize> = (0..self.concepts.len())
        ...
        for rel in &self.relations {
            ...
            out.push_str(&format!(
                "EDGE {src} -[{:?}]-> {tgt} WITH weight: {:.2}",
                rel.relation_type, rel.weight
            ));
            if let (Some(sp), Some(tp)) = (
                self.concepts[rel.source_idx].point.as_ref(),
                self.concepts[rel.target_idx].point.as_ref(),
            ) {
                out.push_str(&format!(
                    "  -- geodesic: {:.4} rad",
                    angular_distance(sp, tp)
                ));
            }
            out.push('\n');
        }
        out.push('\n');
        if let (Some(li), Some(si)) = (
            self.get_concept_idx("language"),
            self.get_concept_idx("sphereql"),
        ) {
            let llm = self
                .get_concept_idx("llm")
                .map(|_| "       PASSING THROUGH llm\n".to_string())
                .unwrap_or_default();
