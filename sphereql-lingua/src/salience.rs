...
impl Default for SalienceScorer { fn default() -> Self { Self::new() } }

impl SalienceScorer {
    pub fn new() -> Self {
        Self { r_min: 0.15, r_max: 1.0, commonality: Self::default_commonality() }
    }
    pub fn score_all(&self, concepts: &mut [Concept], text: &str) {
        if concepts.is_empty() { return; }
        let raw: Vec<f64> = concepts.iter().map(|c| self.raw_score(c, concepts, text)).collect();
    ...
    fn raw_score(&self, concept: &Concept, all: &[Concept], text: &str) -> f64 {
        ...
        let s = [self.frequency_signal(concept), self.position_signal(concept, text),
                 self.idf_signal(concept), self.centrality_signal(concept, all, text),
    ...
    fn position_signal(&self, c: &Concept, text: &str) -> f64 {
        ...
        let mut em = 0.0_f64;
        if lo.contains('?') { em = em.max(0.2); }
        if lo.contains(':') { em = em.max(0.15); }
    ...
    fn centrality_signal(&self, c: &Concept, all: &[Concept], text: &str) -> f64 {
        ...
        for pos in &c.positions {
            ...
            let wt = &lo[s..e];
            for o in all {
                if o.normalized != c.normalized && wt.contains(&o.normalized) { co += 1; break; }
            }
        }
    ...
    fn discourse_signal(&self, c: &Concept, text: &str) -> f64 {
        ...
        let t = &c.normalized;
        let mut sc = 0.3_f64;
        for m in &["end goal","goal:","the point","the question","the method"] {
            if let Some(mp) = lo.find(m) {
                let s = mp.saturating_sub(100); let e = (mp+m.len()+100).min(lo.len());
                if lo[s..e].contains(t) { sc += 0.3; break; }
            }
        }
        for m in &["i think","i want","i suspect","we need"] {
            if let Some(mp) = lo.find(m) {
                let e = (mp+m.len()+150).min(lo.len());
                if lo[mp..e].contains(t) { sc += 0.15; break; }
            }
        }
        let pc = lo.split("\n\n").filter(|p| p.contains(t)).count();
        if pc > 1 { sc += 0.1 * (pc as f64).min(3.0); }
        sc.min(1.0)
    }
    fn default_commonality() -> HashMap<String, f64> {
        [("language",0.75),("system",0.80),("meaning",0.70),("domain",0.65),
         ("hierarchy",0.50),("graph",0.55),("conversation",0.70),("english",0.65),
         ("mathematics",0.55),("algorithm",0.45),("sphere",0.50),("projection",0.50),
         ("representation",0.55),("globe",0.60),("theta",0.35),("phi",0.35),
         ("radius",0.40),("abstraction",0.40),("salience",0.25),("llm",0.30),
         ("sphereql",0.05),("sphere of spheres",0.05),("sphereql space",0.05),
         ("sphereql units",0.05),("sphereql query",0.05),("semantic neighborhood",0.10),
         ("ontological hierarchy",0.08),("epistemic weight",0.05),
         ("abstraction level",0.15),("domain angle",0.08),("system of components",0.25),
         ("oncology",0.20),("cardiology",0.20),("disease",0.50),
         ("pancreatic adenocarcinoma",0.05),("stage-3 pancreatic adenocarcinoma",0.02),
         ("contract law",0.15)].iter().map(|(k,v)| (k.to_string(),*v)).collect()
    }
}
